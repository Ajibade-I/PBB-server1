...
async function validateCharityUpdate(data) {
  const schema = yup.object().shape({
    name: yup.string().min(5).max(50).label("Name"),
    goal: yup.string().label("Goal amount"),
  });
  try {
    const validateData = await schema.validate(data);
    return null;
...
module.exports.validateCharityUpdate = validateCharityUpdate;
